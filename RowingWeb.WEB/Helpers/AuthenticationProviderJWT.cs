...
namespace RowingWeb.WEB.Helpers
{
    public class AuthenticationProviderJWT : AuthenticationStateProvider, ILoginService
    {
        ...
        public async Task GuardarValor(string key, string value)
        {
            await _jSRuntime.SetLocalStorage(key, value);
        }

        public async Task<string> RecuperarValor(string key)
        {
            var u = await _jSRuntime.GetLocalStorage(key);
